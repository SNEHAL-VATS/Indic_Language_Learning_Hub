...
COLORS = {
    'English': '#6a0000',  # Red
    'Kannada': '#004100',  # Green
    'Hindi': '#020263',    # Blue
    'Tamil': '#300038' ,   # Purple
    'Malayalam' : '#ffa500'#Orange 
}
...
st.title('Indic Language Learning App 🌍📖🎓')
...
def set_bg_color(lang):
    color = COLORS.get(lang, COLORS['English'])  # default to English color if not found
    st.markdown(
        f"""
        <style>
        .stApp {{
...
# Call the function to set the background color based on the selected language
set_bg_color(language)
...
def show_messages(mesg_1, mesg_2, message_counter,
    ...
    for i, mesg in enumerate([mesg_1, mesg_2]):
        ...
        # (this time delay only appears when generating the conversation script for the first time)
        if not batch:
